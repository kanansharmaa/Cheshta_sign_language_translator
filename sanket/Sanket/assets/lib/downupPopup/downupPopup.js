...
(function ($) {
    $.fn.downupPopup = function (options) {
        ...
        if (!$this.hasClass("downupPopup")) {
            ...
            if (typeof options === "object" || !options) { // obje olarak geliyorsa $().downupPopup({deneme:"deneme"}); veya boЕџsa
                // Default ayar
                var settings = $.extend({
                    duration: "300",
                    animation: "ease",
        ...
        if (typeof options !== "object") {// obje deДџil de dГјz yazД± geliyorsa $().downupPopup("open");
            if (options === "open") {
